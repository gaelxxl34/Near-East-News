...
class UserArticleController extends Controller
{
        ...
        public function UserPublishedArticleList()
        {
            try {
                ...
                if (!empty($articleData)) {
                    ...
                    return View::make('home', ['message' => 'No Articles Found']);
                }
            } catch (\Exception $e) {
        ...
        public function UserRegionsArticles()
        {
                   try {
                       // Initialize Firestore
                       $firestore = app('firebase.firestore')->database();
                       ...
                       'Algeria', 'Iran', 
                       'Iraq', 'Israel', 
                       'Palestine', 'Palestine (Occupied Territoties)',
                       'Jordan', 'Libya', 
                       'Bahrain', 'Qatar', 'Yemen', 
                       ...
                       foreach ($regions as $region) {
                           // Fetch published articles from the specified region
                           $articlesQuery = $firestore->collection('Articles')
                                                      ->where('Published', '=', 'yes')
                                                      ->where('region', '=', $region);
                           $articleSnapshot = $articlesQuery->documents();
                       ...
                       if (!empty($articleData)) {
                           $articlesByRegion[$region] = $articleData;
                       }
            }
               
                   return View::make('user pages.regions', ['articlesByRegion' => $articlesByRegion]);
                   } catch (\Exception $e) {
                       return 'Error: ' . $e->getMessage();
                   }
        ...
        public function CategoriesArticles()
        {
            ...
            return View::make('user pages.categories', ['articlesByCategory' => $articlesByCategory]);
            } catch (\Exception $e) {
                return 'Error: ' . $e->getMessage();
            }
        ...
        {
            // Assume HightLight UserPublishedArticleList() and TopNews() are modified to return their respective data arrays instead of a view
            $nonePublishedArticles = $this->UserPublishedArticleList();
            ...
            $data = [
                'articles' => $nonePublishedArticles['articles'] ?? [], // From UserPublishedArticleList
    ...
    public function fullArticle($articleId)
        {
            try {
                ...
                if (!$userSnapshot->isEmpty()) {
                    ...
                    return view('user pages.fullarticle', [
